...
func (d *ResourceDetector) propagateResource(object *unstructured.Unstructured,
	objectKey keys.ClusterWideKey, config map[string]interface{}) error {
	...
	if claimedNamespace != "" && claimedName != "" {
		return d.getAndApplyPolicy(object, objectKey, config, claimedNamespace, claimedName)
	}
	...
	if claimedName != "" {
		return d.getAndApplyClusterPolicy(object, objectKey, config, claimedName)
	}
	...
	if propagationPolicy != nil {
		...
		return d.ApplyPolicy(object, objectKey, config, propagationPolicy)
	}
	...
	if clusterPolicy != nil {
		...
		d.RemoveWaiting(objectKey)
		metrics.ObserveFindMatchedPolicyLatency(start)
		return d.ApplyClusterPolicy(object, objectKey, config, clusterPolicy)
	}
...
func (d *ResourceDetector) getAndApplyPolicy(object *unstructured.Unstructured, objectKey keys.ClusterWideKey,
	config map[string]interface{}, policyNamespace, policyName string) error {
	...
	return d.ApplyPolicy(object, objectKey, config, matchedPropagationPolicy)
}

func (d *ResourceDetector) getAndApplyClusterPolicy(object *unstructured.Unstructured, objectKey keys.ClusterWideKey,
	config map[string]interface{}, policyName string) error {
	...
	return d.ApplyClusterPolicy(object, objectKey, config, matchedClusterPropagationPolicy)
}
